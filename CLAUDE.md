...
- **YT duration desync detection**: on `onReady` player compares `getDuration()` vs chord JSON duration (`_computeChordDuration`); if `|Δ|/D > 10%` → shows dismissible yellow banner, gates chord/piano/key updates (time+progress still update); if `|Δ|/D ≤ 5%` → fires `POST /api/process/yt-library-learn` to auto-populate `youtube_library_map` (sessionStorage guard, single-shot per hash+vid)
...
- **Beta homepage (non-admin)**: search bar is the single entry. Results click → play. Empty-results branch renders "+ 新增此曲" CTA; pressing Enter opens the add-song modal (drop-zone + YT URL input + monotonic progress bar) via `#betaFabPanel` (centered, with `#betaFabBackdrop` click-to-dismiss). If Enter value matches `_YT_URL_RE` → fast path: pre-fill URL and auto-click 分析. Homepage also shows wrapping grid library + analysis history (no standalone FAB button anymore)
