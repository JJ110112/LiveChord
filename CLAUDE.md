...
- **Mobile toolbar touch target**: `.tb-trigger` on `max-width: 640px` enforces `min-width: 36px; min-height: 32px` so text-only states (Jazzify "✨AI", Speed "1x", A-B) keep a finger-friendly hit area — without this, tapping the narrow text caused missed taps that looked like the button "stopped working" after entering certain states
