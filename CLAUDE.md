...
- **Long-running operations (>5s) MUST be loosely coupled from the client** — never make the user's browser sit on a synchronous request waiting for a backend job that takes tens of seconds. The pattern (canonical example: [backend/beat_upgrade_queue.py](backend/beat_upgrade_queue.py) + `POST /api/process/upgrade-beats` + `GET /api/process/upgrade-beats/status`):
  ...
  3. **Daemon worker thread** processes the job (fresh in-memory queue per concern; reuse `process_queue` only when the work IS audio-ingest). **Critical: if the worker runs pure-Python loops (numpy, librosa, ML) directly on its own thread, it holds the GIL and starves every FastAPI request thread → "前端無回應"** (experienced on Phase 11b melody extraction). **Must isolate heavy work in a `ProcessPoolExecutor(max_workers=N)` spawned lazy** so each task runs in a fresh interpreter with its own GIL. Pattern: [beat_upgrade_queue.py `_process_one` L128](backend/beat_upgrade_queue.py#L128) + [process_queue.py `_melody_subprocess_extract` L1034](backend/process_queue.py#L1034) + [melody_extract_queue.py `_subprocess_extract` L124](backend/melody_extract_queue.py#L124). This applies even when the daemon thread is already off the event loop — daemon thread != GIL-safe for CPU-bound Python
...
- **Homepage gear settings menu** ([index.html `#btnHeaderSettings`](frontend/index.html) + [base.css `.header-nav` + `.hn-row`](frontend/css/base.css)): topbar collapses Help / Language / Install / Sign-out into a single ⚙ trigger. Theme picker lives here (global setting); player.html no longer carries `.theme-opt` buttons. Popup uses `var(--lc-popup-bg)` + `var(--text)` so it inherits the active theme. The former Sponsor / Buy Me a Coffee entry and `/sponsor` page were removed on 2026-05-20 to match the current non-commercial hobby status. Search-bar magnifying glass uses an SVG `mask` with `currentColor` so it renders monochrome on every theme/OS
- **Scale Lab** ([frontend/js/scale-lab.js](frontend/js/scale-lab.js) — shared by homepage `#secScaleLab` and the player key badge): one self-contained module (`window.ScaleLab`) owns the scale catalogue (31 scales / 5 categories with 音色特性 + 介紹), enharmonic spelling, the piano / fretboard canvases, TAB text and a bare-WebAudio preview (no Tone.js — player.html doesn't load Tone). Player: clicking `#chordKey` calls `ScaleLab.openModal({key, mode})` using the badge's `data-mobile-key` (current key during a modulation) + `data-mode` (set in `_updateKeyDisplay` and both chord-load sites) so Dorian / Mixolydian songs open the matching mode, not plain major/minor. The player surface is a **Type C floating panel** (`.scale-panel.lc-panel`, no backdrop, closed only by ×) that **follows the song**: every `_updateKeyDisplay` exit calls `_syncScaleFollow(key, mode)` → `ScaleLab.followKey()`, which de-dupes on `root|scaleId` so a user's manual scale pick inside the panel survives playback ticks until the key actually changes. Shared `.sl-*` CSS lives in base.css (both pages load it) — bump base.css `?v=` on BOTH index.html and player.html. Plan + spelling rules: [doc/plans/scale-lab-plan.md](doc/plans/scale-lab-plan.md)
...
- **Served-melody honesty (`melody_quality`, LiveChord-a1lh)**: `/api/ai/melody` responses carry `melody_quality` = [ai/melody_trust.py](backend/ai/melody_trust.py) verdict (`trusted`, `reason` ∈ full_mix_ok / vocal_stem_crepe / too_few_notes / bass_leak / low_coverage / vocal_gate_refused, `coverage`, `low_fraction`). On `bass_leak` the server drops notes more than an octave below the median before serving (`quality_flags += bass_leak_notes_dropped`, cache untouched) — the player used to octave-fold them UP into the melody, which was the「右手旋律夾雜左手低音」complaint. Player `_noteMelodyQuality` toasts once per song (`toast.melody.unreliable.<reason>`) for bass_leak / low_coverage / too_few_notes when the RH mode shows melody. Resolver v0 also promotes cached `solo_piano_polyphonic` then `instrument_lead` when the vocal gate refuses (human A/B: 7/9 and 5/14 wins over pYIN) — but only 9 + 6 such candidates exist on disk; instrumentals need a candidate batch before this matters
...
3. `meter_regularizer` — 6/8 and 3/4 only. Cleans the tracker's half-bar pulse list, picks bar heads by DP over the half-bar grid (2 pulses per bar, 1 or 3 allowed at a cost, maximising chord-onset hits — a held phrase ending that adds an odd half-bar becomes a local phase flip, not a half-bar shift of every later chord), rebuilds beats[] as exact subdivisions per bar, quantizes chords to the half-bar grid. **整小節優先**: when ≥70 % of changes fall on bar lines OR ≥75 % of time is in bar-long chords, re-quantize at whole bars with a no-drop guard (any chord ≥ ~half a bar keeps the bar it overlaps most; rescued bars are locked so one rescue cannot overwrite another). Player 6/8 card dots count the regularized beats inside the card, not tempo_curve.
   - **Compound-meter felt BPM**: `bpm` in the chord JSON counts the tracker's beat unit (eighths for 6/8 — Libera reads 130.4). Anything that judges tempo or picks a style must convert to the felt pulse: `felt = bpm × practice_pulses_per_bar / display_subdivisions_per_bar` (6/8: ×2/6 → 43.5, 慢板). `progression_pattern.describe_style` does this (fields `bpm` = felt, `bpm_raw`, `bpm_note`) and passes `time_signature` to `suggest_style`; the player topbar already shows the felt value. Never feed the raw eighth-note BPM to `suggest_style` or a tempo label — it turned a serene choir piece into 快板 with a "Rock 八分" suggestion.
...
## AI Accompaniment (engine v9, deployed 2026-09-06)

LH/RH accompaniment in [backend/ai/accompaniment_generator.py](backend/ai/accompaniment_generator.py) — 21 styles, 7 piano RH modes, multi-instrument (piano / guitar / ukulele / accordion / arranger). Output cached at `data/accompaniments/{hash}_{style}_{level}_{section_type}_{instrument}_{ACC_ENGINE_VERSION}.json`; engine version `v10` (bump to invalidate; v8→v9 on 2026-09-06 when note continuity went active and pattern-truncation slivers < 50 ms were dropped at emit time; v9→v10 the same day for the `melody_trust` gate below). Architectural detail in [doc/for-notebooklm/](doc/for-notebooklm/).
...
- **`melody_trust` gate (ai_api.py, v10)**: the accompaniment endpoint plans around the resolver-served melody only when it is plausibly the lead line — `vocal_stem_crepe` always; `full_mix_pyin` only if the vocal gate did not refuse, ≥ 20 notes, active coverage ≥ 40 % of the song, and ≤ 30 % of notes below MIDI 55 (bass-leak signature). Untrusted → `melody=[]` for Auto RH vocal-avoidance, LH collision dedupe and pedal; decision stamped as `result.melody_trust`. Bit us on Unchained Melody: a bass-contaminated pYIN melody (107/206 notes < MIDI 55) made RH play only 0.5 s fills and the dedupe drop half the LH bass (LiveChord-gyjt)
...
- **Note continuity is feature-flagged and ACTIVE on the NUC since 2026-09-06**. `LIVECHORD_NOTE_CONTINUITY_MODE` or `data/settings.json.note_continuity_mode` accepts `off` / `shadow` / `active`; the code default is still `shadow`, the NUC settings file says `active`. Shadow mode writes event-level `continuity_meta.would_*` and top-level `continuity_observation` without changing durations; active rewrites `duration` and records `continuity_meta.extended_by`. Phase 2.5 gate was approved on the 411-cache shadow review (0 overlaps, median extension 0.023 beat; the only defect was the truncation-sliver case, fixed in v9). Mode changes require uvicorn restart; to backfill cache after a mode flip, use `/api/admin/accompaniment/recompute` or bump `ACC_ENGINE_VERSION`.
...
- **`ChoralWorld` (22nd style, 2026-09)** — heavenly flowing accompaniment for choir / relax / sleep material (Libera). LH: six evenly spaced soft touches per pattern, root → 5th → octave rising and falling (`STYLE_DICT["ChoralWorld"]`), `lh_vel 52 / rh_vel 62`, `rh_mode "arpeggio"`, in `_DOWNBEAT_STYLES`, string idiom `arpeggio`. **It is the only style whose LH `pattern_period_beats` depends on the meter**: `_build_left_hand(..., time_signature=)` uses 6 for 6/8 (one pattern = one bar of six eighth beats), 3 for 3/4, otherwise the configured 4 (sextuplet feel in 4/4). Suggested first by `suggest_style` for genres containing choral / choir / relax / sleep / new age / world, and boosted +3 in 6/8 / 12/8 when the felt BPM is under 60. Pickers: player `#teachStyle` optgroup "Choral / World", Progression Library group「古典 / 合唱」, `styleNames` map in player.js. New style names get their own cache files — no `ACC_ENGINE_VERSION` bump needed when adding a style.
