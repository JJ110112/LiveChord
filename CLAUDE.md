...
- **VexFlow gotchas** (all bit us, all costly to rediscover):
  ...
  - Voice duration MUST equal `num_beats` exactly. Off-by-1/16-beat throws `RhythmException` and crashes the layer. `_buildBarNotes` includes a Padding Patch: after quantization, if `sum(note durations) ≠ tsBeats`, append rests (positive diff) or truncate the last note (negative); if still off by > 0.001, replace the bar with a whole-bar rest fallback
...
- **Score density / layout**:
  ...
  - Pitch-range filter in `_buildBarNotes` drops out-of-range pitches BEFORE rendering (bass: MIDI 33–72 / A1–C5; treble: 53–100 / F3–E7). Ledger lines are KEPT for in-range notes — user wants them as a reading scaffold, NOT octave-clamped (silent shift broke pitch correspondence with keyboard, rejected)
