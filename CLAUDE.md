...
LH/RH accompaniment in [backend/ai/accompaniment_generator.py](backend/ai/accompaniment_generator.py) — 21 styles, 7 piano RH modes, multi-instrument (piano / guitar / ukulele / accordion / arranger). Output cached at `data/accompaniments/{hash}_{style}_{level}_{section_type}_{instrument}_{ACC_ENGINE_VERSION}.json`; engine version `v9` (bump to invalidate; v8→v9 on 2026-09-06 when note continuity went active and pattern-truncation slivers < 50 ms were dropped at emit time). Architectural detail in [doc/for-notebooklm/](doc/for-notebooklm/).
