...
| 8a | Gate threshold re-calibration (2026-09-02): a 200-song random candidate batch exposed six instrumentals (sax / flute / harmonica / brass) in the 0.06-0.15 band that the validation set never contained. The 34-song gate-band A/B review (`phase0_5_ab_gate_band_20260902`, `/melody-ab?queue=gate_band`) found 0/6 CREPE wins below 0.15 and 28/29 above. Default moved to 0.15 (`vocal_stem_ratio_gate_v2`); validation-set recall drops to 0.896 (precision 1.0), accepted because a false negative only restores the pre-resolver pYIN output | Done |
| 9 | Vocal route ship audit Stage B: confusion matrix on held-out labels, `vocal_led` precision >= 92%, 30% retreat gate metrics, and 12-song vocal residual metrics | Done: leakage-clean vocal gate passed at threshold 0.06; resolver v0 is wired for cached `vocal_stem_crepe` promotion with two runtime retreats (absolute vocal-stem ratio/duration gate plus relative active-duration/density <30% vs `full_mix_pyin`) and public-mode bypass; 12-song vocal residual report passed both 30% post-hoc QA thresholds while still surfacing 5 low worst-window songs for targeted listening. Post-ship drift monitoring is a follow-up, not a blocker |
| 9a | Solo piano Stage 2.1 selector: add hand-state continuity prior for crossed/interleaved-hand textures | Implemented: `piano_rh_selector` now uses a small beam search with recent-pitch hand-state scoring, so brief candidates more than an octave away from the running RH trajectory are penalized without removing skyline alternatives. The 9 solo-piano smoke candidates were regenerated with the new selector; next action is human A/B review of Debussy / Tifa / other prior weak cases |
| 10 | Add Phase 0 survey summary/report output: completion count, primary-tag distribution, post-filter-fixable ratio, secondary/audio-quality breakdown | Done: `build_survey_report()` powers `tools/report_melody_phase0_survey.py` and `/api/ai/melody/debug/survey/report`. Current 200-song queue report is generated at `V:\data\melody_reviews\phase0_survey_report.json` with 0/200 completed, so the report path is ready before human tagging begins |
