...
**㉑ 21:49 take：移調又是 0 —— 不是壞了，是回聲活不到和弦換過去**

整趟很乾淨：人類音 423、生成音 610、丟棄 0、靜音 0、迴圈 0、jitter p95 1.84 ms（max 2.99 ms）。**另存即採用的修正確認上機**：`saved` 事件寫的是 `{"path":"test01.json","scene":"test01"}`，與載入的 scene 一致，下一次留空存檔不會再寫到別的檔。

但 `phrase_modx` 觸發 16 次、`phrase_shift` **0 次**，和前一趟的 4 次矛盾。把這趟的 HUMAN 事件整段餵回引擎重放（重現 17 次觸發 / 1 次移調），在 `_phrase_target` 第一次被呼叫時記錄「捕捉到的和弦」對「當下的 `chord_solid`」：

```
 觸發時刻  捕捉和弦  pass數  回聲存活   下一個和弦   多久後才換
   46.45   E          2     1.86s     Am            1.2s   ← 唯一一次移調
   57.26   Am         1     0.74s     G             4.4s
   72.09   Am         1     0.66s     B             2.4s
  152.26   C7         2     0.79s     C             2.4s
  161.69   Fm7        2     0.99s     Ddim          2.0s
  183.06   Dm9        2     2.49s     Fm            2.7s
  186.03   Fm6        1     0.78s     Ddim          1.9s
```

**回聲整段壽命中位數 0.78 s，演奏者換和弦的中位數 2.7 s。** 答句在和聲移動之前就結束了，所以每次去問「現在是什麼和弦」，答案永遠還是捕捉時那一個 —— 沒有東西可以移調。唯一成功的 46.45 正是因為它活了 1.86 s、而和弦 1.2 s 就換掉。機制沒有壞，是幾何不對。

壽命這麼短的原因是 `decay: 0.55`：`repeats` 雖然是 4，但整趟以「最小聲的音」判定的 all-or-nothing 門檻在第 3 趟就跌破 `min_vel`，實際只出得來 1–2 趟。

**A/B（同一段演奏重放，只掃 `decay`）**：
...
| 0.75 | 17 | **6** | 233 |
| 0.85 | 17 | 13 | 322 |
| 0.85 + repeats 6 | 17 | 17 | 370 |

使用者選 **0.75**：回聲拉長到約 4–5 s，剛好跨過換和弦的 2.7 s 中位數，移調 1 → 6 次，出音量只多 58 %；0.85 移調更多但 MODX 明顯變吵（出音多一倍以上）。已寫進 `test01.json` 的 live 邊。

**留著的一顆地雷**：`test01` 的 preset A 仍是 `decay 0.55`，按下 A 會把這個設定叫回舊值。這是使用者自己存的快照，沒有替他覆寫；下次他滿意 0.75 時再存一次 A 即可。

MIE 測試 162 個。

**㉒ 22:16 take：「一直有個高兩個八度的 mi 長音，有點干擾」—— sustain 被自己的天花板釘死了**

`decay 0.75` 上機：`phrase_shift` **3 次**（前一趟 0），觸發 20 次。整趟人類音 427、生成音 732、丟棄 0、靜音 0、迴圈 0、jitter p95 1.74 ms。

那個 mi 是 **`sustain_strings`（CH4 弦樂）**。26 個音全部落在 B4–E6，中位數 B5，長度 5.8–12 秒、每 4–5 秒發一次 —— 前後重疊，**這條 lane 有 82 % 的時間都在響**。其中 E5 三次、E6 三次，而 E6 = 88 正好是這條邊的 `high`。

原因在 `sustain._candidates`：
...
1. **`st.sounding` 是「手指 + 踏板」**。踏板踩著的時候它只會往上長 —— 演奏者往上摸一次 A6，這個地板就被釘在天花板，接下來十幾秒都下不來。這一趟 26 次觸發裡有 **10 次** 的 `low` 被夾到 76。
2. **夾到 `high - 12` 並沒有「讓開」**。演奏者已經彈在這條邊的頂端八度時，「在他上面」根本做不到；把 low 夾到 76 只是把整條 lane 塞進 76–88 這一個八度、然後留在那裡。而 **E 正好同時是這個窗口的下緣（76）與上緣（88）**，所以它一直回到 mi。

改法兩條，都是「這個地板存在的理由是不要坐進演奏者的和弦裡」的直接推論：讀 `held`（現在按著的）而不是 `sounding`；而且只在**還有空間可以在上面**時才抬地板，否則就用邊自己的音域 —— 人在頂端時，讓開的方向是往下。

**A/B（同一趟演奏重放，同 seed）**：

| | p25 | 中位數 | p75 | 最高 | ≥E5 | mi 的數量 |
|---|---|---|---|---|---|---|
| 修改前 | E5 | A5 | C6 | E6 | 78 % | 5 |
| 修改後 | A#4 | F5 | B5 | D6 | 60 % | **1** |
| 修改後 + `above_held:false` | E4 | G4 | D5 | B5 | 20 % | 3 |

最能說明問題的是尾段：修改前是 `B5 D6 E6 C6 C6 E6 E6 A5 A#5`（不間斷的高音 drone，四個 mi），修改後是 `A5 B5 D6 A5 C4 F4 A#4 A#5 A#4` —— 它會回得來了。

兩個回歸測試：踏板放掉的高音不能把 pad 釘在上面；演奏者彈在頂端時 pad 不能把自己塞進頂端八度。兩個在舊程式碼上都會失敗。

若使用者仍覺得偏高，`above_held: false` 會再降一個八度（中位數 G4）—— 那是口味旋鈕，不是 bug。
...
**㉔ 22:42 take：三個修正全部上機確認；`repeats` 4 → 3；面板兩個防呆**

目前最乾淨的一趟：341 人類音 / 795 生成音、丟棄 0、靜音 0、迴圈 0、**錯誤 0**、jitter p95 **1.47 ms**（歷來最低）。

三個修正逐一驗證：**純三和弦被加大七度 0 次**（前一趟兩個 6.8 秒的 B5）；`sustain_strings` 中位數 **A4**、範圍 D4–E6（不再貼天花板）；`phrase_shift` **10 次**（0 → 4 → 10）。

**密度**：每個人類音對到 **2.33** 個生成音，前幾趟是 1.44–1.71。拆開來是兩個獨立原因：每次樂句回聲的音數因 `decay 0.75` 從 8 個變成 12–13 個（我改的，預期內），而樂句觸發次數 16 → 32 是演奏者這趟彈得比較稀疏、句子間停頓變多，偵測器照著他的呼吸走。

使用者選擇把 `repeats` 4 → 3。**A/B（同一趟重放，重放本身是 29 次觸發 / 336 音 / 11 次移調，與實機 32 / 383 / 10 相符）**：

| 設定 | 觸發 | 移調 | phrase 音數 | 每次音數 |
|---|---|---|---|---|
| decay 0.75 / repeats 4 | 29 | 11 | 336 | 11.6 |
| **decay 0.75 / repeats 3** | 29 | **9** | **313** | 10.8 |
| decay 0.75 / repeats 2 | 29 | 4 | 254 | 8.8 |
| decay 0.55 / repeats 4 | 29 | 3 | 210 | 7.2 |

**只少了 7 %** —— 因為 `decay 0.75` 下第 4 趟本來就常常跌破 `min_vel`。真要再稀，下一格是 `repeats 2`（−24 %，但移調從 9 掉到 4）。

**順帶量到：`prob` 對 phrase lane 幾乎沒有作用。** 0.8 → 0.5 讓 skip 從 53 變成 90，觸發次數卻是 29 → 31。原因是 `phrase.on_skip` 直接把 `answered` 清成 None、沒有任何重試間隔，所以擲骰輸掉的下一個 tick 就重擲，幾毫秒內一定會贏。sustain 的 `on_skip` 用 `retry_beats` 等一下，phrase 等 0。這是設計上「不要讓一句被骰子吃掉」的直接後果，但代價是這個旋鈕形同虛設 —— 尚未決定要不要給它一個 `retry_beats`。

**面板兩個防呆**（`mie.js?v=28`）：

1. **XY pad 只送有動的那一軸**。原本每 33 ms 兩軸都送，一次拖曳把 log 灌滿沒有變化的 set —— 22:29 那趟 `every_bars_min: 1` 在 800 ms 內被記了五次，而實際在動的只有 `voices`。要從 log 看出演奏者做了什麼時，這種雜訊很礙事。
2. **`every_bars_min` / `every_bars_max` 納入成對夾制**。使用者在 22:42 那趟把「最長」設成 0.25、低於「最短」0.5；演算法 `hi = max(lo, hi)` 會默默夾住，所以他移動的那條滑桿什麼都沒做、也沒有任何提示。現在兩端互推，跟音域那組一樣。`pushPartner` 也改成依照對方滑桿自己的 step 取整（原本一律 `Math.round`，0.25 一格的參數會被抹掉）。

（同一份 scene 檔裡還帶著使用者自己在面板上調的 `sustain_strings`：`prob 0.25`、`vel 23`、`voices 2`、`every_bars_min 0.5`，以及重新存過的 preset A。）

面板在靜態伺服器上載入無 console 錯誤；pad 與成對夾制要接上引擎才走得到，留待下一趟實機驗收。MIE 測試 165 個。

**㉕ 彈法條件上線：`when: {texture: [...]}` 從「有分類器但沒人用得到」變成面板上的五顆鈕**

使用者說密度那件事「後面會做偵測使用者的彈奏才能解決」，然後去休息，留下「可接續未完成的部分」。這一段就是那條路的第一塊：**織度分類器（quiet / sustained / chord / arpeggio / melody）早就在跑、也早就有測試，`Edge.wants()` 也早就接進 `timed_edges` 與 `candidate_edges` 兩個呼叫點 —— 但沒有任何 scene 寫過條件，面板上也沒有地方可以寫。整個功能是隱形的。**

**這一趟做的：**

1. **`Edge.wants()` 補上測試**（先前完全沒有，而它在熱路徑上）：沒寫條件的邊吃所有彈法；寫了就只吃列表裡的；**沒有讀數（None）要通過**——條件是用來刻意收窄，不是因為分類器一時沒話說就啞掉；空列表 / `{}` / `null` 都等於不限，所以在面板上把鈕全部取消會回到「不限」而不是把這條線永久靜音。兩個呼叫點各驗一次：只條件化其中一個，就是「被叫不要插手的 lane 還是回應了音」的來源。
2. **拼錯的彈法名稱會在列上講出來**。`wants()` 是嚴格的（不在列表裡就不通過），所以一個拼錯的名字會讓這條線永遠不說話 —— 正是 `_mute_reason` 存在的理由。面板只寫得出合法名稱，手改 scene 則不然。順手把 `wants()` 的 docstring 改成與程式行為一致（原本寫「未知的彈法也會通過」，實際上不會）。
3. **面板：每條邊一排「只在這樣彈」五顆 chip**（靜／持續／和弦／琶音／旋律）。全部不選 = 不限，也就是今天所有邊的現況，所以這個功能開起來在使用者動手之前不會改變任何聲音。寫入 `when`，同時把 `texture:` 簡寫清成 null（`wants()` 先讀簡寫，兩個並存的話 `when` 永遠不會被看到 —— 一份狀態兩個名字就是這樣出事的）。
4. **等待狀態說得出口**。條件不符的 lane 不是壞掉也不是沒話說，它是**在等**，所以列上顯示 `N ⏸`、tooltip 寫「這條線只在『和弦』時說話，你現在是『持續』」。能解釋的安靜跟不能解釋的安靜是兩回事。
5. **`--port` 其實從來沒有作用**。面板把 WebSocket 位址寫死成 `ws://127.0.0.1:8810/ws`，所以 `--port 8899` 會讓引擎聽在你指定的地方、面板卻還是找 8810。改成用「送出這個頁面的那個 port」，只有 LiveChord 後端（8800）代管時才退回 8810。
6. **`follow_chord` 的欄位根本綁不上**。它在 scene 裡是 JSON boolean `true`，面板卻用 0..1 的滑桿畫它 —— 數字框顯示空白，每個 snapshot 都吐一次 `The specified value "true" cannot be parsed`（QA 面板一分鐘 85 次），而且真的去拖它會把 boolean 覆寫成數字。新增 `BOOL` 欄位型別（checkbox），順便把 `sustain.above_held`、`silence.follow_chord` 也放上面板 —— `above_held` 正是前天那個「弦樂貼天花板」的元兇，本來只能改檔案。

**驗收方式**：寫了一個不開任何 MIDI port、`send` 接到假輸出的 headless 引擎 + 真的 UiServer（**8811，不是 8810**），所以樂器全程沒有收到任何一個音。在瀏覽器上實測：8 條邊 40 顆 chip；4 個 boolean 欄位都正確帶值；沒有空白的數字框；零 console 錯誤。把 `sustain_strings` 設成「只在和弦」而當下是「持續」——**觸發數在 6 秒內完全沒動（16 → 16），tooltip 說明了原因；按復原之後 chip 回到不限、等待狀態解除，觸發數 7 秒內 17 → 19 恢復。** 收工後確認 8810 / 8811 / 8899 三個 port 都已釋放。

`mie.js?v=30`、`mie.css?v=15`。MIE 測試 168 個。

**還沒動、留給使用者決定的**：`phrase` 的 `prob` 形同虛設（`on_skip` 沒有重試間隔）；`min_notes` 是否該數「起音群組」而不是音數（一個五音和弦目前被當成五個音的樂句）。

**㉖ 第 0 段：log 現在描述得出一個音的長度（視覺化與重放的前提）**

使用者帶回一份外部 UI/UX 提案（鋼琴捲軸視覺化 + 風格預設），核准的順序是 **0 修 log → 1 `mie_replay.py` → 2 視覺化 → 3 風格預設 → 4 Auto detect 只做建議**，並採納三個反對意見（不用 Pixi、不重排版面、Auto detect 不自動切換）。

提案的 Roadmap 第 2 步「Log 資料結構標準化」其實**早就完成**（每個 session 都是結構化 JSONL，還帶 `lane` / `edge` / `hop` / 觸發機率）。但照著它的規格去畫鋼琴捲軸，第一步就撞牆：

**log 記得下每個生成音在什麼時間、什麼音高開始，但沒有記它響多久。**

- `gen` 事件沒有任何長度欄位。
- `sched` 有 `dur_ms`，但那是**排程時**的資料，而 late binding 會在送出前重新吸附音高——實測 22:54 那趟，**852 個音有 46 % 送出時的音高與排程時不同**。所以 `sched` 列無法與真正響出來的音配對，它的 `dur_ms` 屬於一個從來沒響過的音高。
- `off` 只從 `_force_off` 發出，那一趟 **852 個 note_on 對 59 個 off**。

改法：`gen` 帶上送出當下**規劃**的長度（`dur_ms`），Shadow 這種「跟著人放開」的加註 `follow`（規劃值只是安全上限）；`_on_sent` 的 off 分支記下**每一個**釋放，帶 `held_ms`；`_force_off` 也補上 `held_ms`，讓消費端永遠只有一種形狀。

**實測（同一趟演奏重放整個 test01 場景）**：

```
gen 882  off 879          （改之前是 852 gen / 59 off）
有長度的 gen：882 / 882
標記 follow 的：79
每個 off 都有 held_ms：True
規劃長度 vs 實際長度：中位誤差 0 ms
   固定長度的 lane：中位誤差 0 ms   (n=800)
   follow 的 lane ：中位誤差 7785 ms (n=79)  ← 這就是要標記它們的原因
```

**一個要寫進文件、不是 bug 的性質**：882 個音裡有 3 個沒有對應的 `off`。原因是**同一個音高在同一個 channel 上還沒放開就再次被觸發時，兩個 pair 共用一個 note_off**——合成器對一個音高只有一個聲部，多送一個 note_off 會切掉當下正在響的東西。這是正確的 MIDI 行為；讀 log 的人在沒有 `off` 時改用該音自己的 `dur_ms` 即可。同一趟的線上檢查：882 note_on / 881 note_off，PANIC 對兩個 port 的 16 個 channel 各送出 CC120 + CC123 + CC64=0，硬體不會有殘留音。
...
**㉘ 第 2 段：鋼琴捲軸（`mie-roll.js`）—— 時間橫軸、音高縱軸、一個 lane 一個顏色**

一趟 take 是四分鐘、上千個音、七條 lane、六台樂器。事件流一次只能給你最後二十行，那是錯的形狀：真正要問的是「這條 lane 是不是一直坐在我頭上」「這個答句回來的時候和弦換了沒」「弦樂為什麼老是在那麼高的地方」。

**做法（照使用者核准的三個反對意見）**：Canvas 2D 不用 Pixi（這個 repo 的 `player.js` 本來就這樣畫瀑布流，一趟幾千個矩形而已）；**做成收在下面的面板，三欄結構完全不動**（上面那三欄是演出當下工作的地方）；預設收著，按「捲軸」或 `R` 打開，開啟時才建立 canvas，關著的時候 `pushEvent` 不做任何額外的事。

- **顏色依 lane，不依演算法**。scene 自己命名邊（`phrase_modx`、`iridium_to_wavestate`），寫死的圖例一換 scene 就過期；lane 是引擎自己的詞彙。圖例上的色塊就是畫布上的顏色，點一下單獨隱藏，Alt+點只留這一條。
- **載入錄音是一個下拉選單，不是檔案對話框。** UiServer 新增唯讀的 `/api/logs` 與 `/api/log/<name>`，`_log_path` 只接受 `session-YYYYMMDD-HHMMSS.jsonl` 這個字面形狀，其餘一律 404（實測 `../../../CLAUDE.md` → 404）。剛彈完要回顧那一趟，應該是一次點擊，而不是兩手還放在琴上時去開檔案視窗。另外保留「檔案」按鈕載入別處來的 log。
- **舊 log 會講實話**：2026-09-09 之前的錄音沒有音長，載入時直接寫「⚠ 852 個音沒有長度，畫成短棒」，而不是畫一堆它其實不知道的東西。沒有對應 `off` 的音（同音高重觸發共用一個 note_off）用排程長度畫，並在尾端加一條標記。
- **Shift + 拖曳**畫反覆區間、滾輪縮放、0.25×/0.5×/1×/2× 回放、滑過任何一個音顯示 `E4 · 樂句 · phrase_modx · CH12 · v33 · 428ms`。

**QA（headless 引擎，不開任何 MIDI port，UiServer 在 8811）**：

- 載入 22:54 那趟：562 個人類音 + 852 個引擎音，lane 圖例 `human:562 echo:378 phrase:283 follow:120 shadow:59 sustain:10 echo2:2`，長度 4:16。
- **用讀畫布像素驗證繪圖**（比截圖可靠）：全部顯示 53,183 個彩色像素；只留 human 剩 19,589，色相全部收斂到 140–150（human 的色相是 145）；全部關掉剩 628 個，色相 0 —— 那是播放頭。篩選、配色、圖層都對。
- **走時器（transport）拆成 `advance(dt)`，與動畫迴圈分離**後可測：1× 前進 10 秒 = +10.00；0.25× 前進 10 秒 = +2.50；反覆區間 [100,104] 從 103 前進 2 秒 → 回到 100；跑到結尾夾住並自動停止。（`requestAnimationFrame` 在隱藏的分頁完全不會跑，這是對的行為，但也讓「動畫」本身無法在 QA 環境裡驗——所以把數學拆出來自己驗。）
- **即時模式**：讓 headless 引擎持續彈一段樂句，6 秒 30 個音 → 12 秒 77 個音，播放頭跟著走。**80 個生成音全部帶著長度**（第 0 段的成果），76 個有真正的 `off`；實測 echo 0.344 s（排程 0.343）、follow 1.053（1.052）—— 排程與實際吻合；有一個 sustain 實際只響 1.472 s 而排程是 5.609 s，被中途切掉了，這正是這張圖該讓人看見的東西。

**順手修掉的兩個問題**：圖例每點一次就重建整個列表，等於把手指下面那顆按鈕換掉，連點會漏掉（改成只更新 class）；開啟捲軸時自動捲到視野內（它在三欄下面，開了看不到等於沒開）。

`mie-roll.js?v=5`、`mie.js?v=33`、`mie.css?v=16`。MIE 測試 171 個。

**㉙ 第 3 段：介入風格預設 —— 把已經存在的東西打包命名，不新增任何引擎行為**

`data/mie/styles.json` + 頂列一個下拉選單。五個風格：敬拜／氛圍、爵士／和聲擴展、流行／抒情、藍調／R&B、留白。

**兩個設計決定**：

1. **依演算法命名，絕不依邊 id。** scene 自己命名邊（`phrase_modx`、`iridium_to_wavestate`），寫死那些 id 的風格只會合一台琴，換一個 scene 就靜默地什麼都不做。測試裡直接用兩條叫 `whatever_i_called_it` 與 `and_this_one` 的邊驗這件事。
2. **一個風格不新增任何引擎行為。** 它動的每一格都是使用者本來就能自己拉的：tension、density、time、prob_scale、restraint，以及每個演算法的 constraint / vel / hold / decay / voice_lead。所以中場伸手去按它是安全的，而且按下去之後每一根滑桿都看得見它動去哪裡。

**套用會先把「你自己的設定」存起來**，取消就整組回來——**不是回到上一個風格**。而且風格之間直接切換也會先回到那個基準再疊上去：實測 worship 設 `hold_beats 12`、sparse 沒提到它，沒有這一步的話切到「留白」會留著十二拍的長音，那正是「留白」最不該有的東西。

**A/B（22:54 那趟重放，同 seed）**：

| 風格 | 生成音 | 每個人類音 | 丟棄 | 移調 |
|---|---|---|---|---|
| 留白 | 230 | **0.41** | 0 | 0 |
| 敬拜 / 氛圍 | 512 | 0.91 | 0 | 0 |
| （目前 scene） | 864 | 1.54 | 4 | 6 |
| 流行 / 抒情 | 1045 | 1.86 | 0 | 7 |
| 爵士 / 和聲擴展 | 1145 | 2.04 | 0 | 9 |
| 藍調 / R&B | 1277 | **2.27** | 3 | 7 |

五個風格都是 0 靜音、0 迴圈、0 錯誤——沒有一個把引擎推到安全層的界線。`--styles` 也加進 `mie_replay.py`，所以任何一趟演奏都可以自己量一遍。

**做的過程中抓到三個「設定會被靜默忽略」的問題**，這是這個專案付過最多代價的那一類 bug：

1. **TIME 會量化到音樂比例**（¼ ⅓ ½ ⅔ 1 1½ 2 3 4）。敬拜寫 1.8 → 落在 2，跟目前 scene 的 2.05 一模一樣；藍調寫 0.85 → 落在 1，跟爵士一樣。兩個風格的「比較慢」與「比較緊」都是空的。改成寫真正的級距值。
2. **`"scale": "blues"` 完全沒有作用** —— 音階是從調式推導的，沒有任何地方讀這個 global。移除，並把藍調的說明改成老實話：它的顏色來自 tension 0.9（變化音）與 follow/shadow 的 `constraint: scale`，**不是**強制藍調音階。（真正的「指定音階」需要一個新的 global，列為後續。）
3. **取消風格會漏掉風格新加的 global。** `_apply_settings` 只能寫它手上有的 key，所以風格引進的 `tension` / `density`（scene 原本沒有的話）會留在那裡，「取消」等於沒取消。改成先刪掉多出來的 key 再還原，並重新同步 density / time / safety。

**一個測試擋住整類問題**：`test_every_style_only_sets_things_the_engine_actually_reads` 逐一檢查每個風格寫的每一個 key —— global 必須在 `KNOWN_GLOBALS` 裡，演算法參數必須是 Edge 的宣告欄位或該演算法原始碼裡真的出現過的字串。實測注入 `groove` 與 `swing` 兩個假設定，測試立刻指名道姓地擋下來。

**這些數值是起點，不是主張。** 我聽不到那些樂器；量得出密度與音域，量不出「像不像敬拜音樂」。每個風格都彈過再自己拉滑桿，拉完滿意就存進 A 或 B。

MIE 測試 174 個。`mie.js?v=34`、`mie.css?v=17`。

**㉚ 第 4 段：落差提示（Live Advisory）—— 說出可量測的落差，不猜曲風、不自己動手**

原提案要偵測**曲風**再自動切換。量過六趟之後這件事做不到：texture 讀數在每一趟都被 `quiet` 與 `melody` 佔滿，和弦品質大多是 `5` / `(3)` / `m(3)` 這種兩音片段（手指起落造成的閃爍）。那些特徵分不出敬拜、爵士、流行 —— 拿去猜就是猜，然後包裝成智慧。

但同一份資料支援另一件事：**說出「你在做什麼」跟「引擎被設定成怎麼回應」之間的落差**。每一條都是一個事實加上一個具名的控制項，而且**每個門檻都是從使用者自己的錄音讀出來的**：

```
take               rich%   gen/human   tension   最高重疊 lane
...
- **訊號 A 和聲張力落差**：豐富度四趟落在 8–10%、兩趟 36–42%，而 tension 六趟都是 0 —— 那兩趟引擎在拒絕使用者真正彈出來的和聲。
- **訊號 B 密度落差**：2.33 正是使用者自己說「整體變密了」那一趟，門檻就設在那裡。
- **訊號 C 音域重疊**：提案要求「所有 lane 超過 80%」—— **每一趟都超過**，因為 Shadow / Echo / Phrase / Follow 的音高本來就取自演奏者，重疊是它們的工作。這個燈會永遠亮著，等於沒有訊號。改成**只看會自己挑音域的演算法**（sustain / silence）。第一版我用「排除清單」寫，結果 `phrase` 漏進來、六趟裡有五趟被誤報 —— 改成**允許清單**（依演算法，不依 lane 名稱）之後乾淨了。

**驗證方式是把六趟重放一遍，看它會說什麼、什麼時候說**。修正後：密度在 5/6 趟觸發（2.1–3.1）、張力落差在 5/6 趟觸發（25–38%），而 **sustain 的音域重疊在 22:16 那趟 t=92 秒觸發 —— 那正是使用者說「一直有個高兩個八度的 mi 長音，有點干擾」的那一趟**。

**兩個防噪機制**：`confirm()` 要求同一條建議**連續兩次讀數都成立**（相隔 8 秒）才顯示 —— 45 秒的滑動窗口會讓臨界值上下跳，一閃而過的建議比不說話更糟；以及慢速 lane 的取樣門檻降到 5 個音（pad 每 5–10 秒才說一次話，12 個音的門檻會讓這個訊號只對快 lane 有效，而那正是它不該作用的地方）。

**它只會說，永遠不動手。** 每一條建議帶著確切的 `(path, value)`，按下去就是一次普通的 `set` —— 滑桿會動、事件流看得到、**Ctrl+Z 退得回來**。實機驗證：張力 0 → 按「把張力調到 0.5」→ 0.5 → Ctrl+Z → 0。有一個測試直接斷言「彈三十秒之後，所有 global 與所有邊的設定一字未變」。

四個測試：不得因為 lane 在做它的工作而告狀、一次讀數不算數、事實與修正值要對得起來、以及「它自己什麼都不會改」。

順手修掉：事件流裡 `human_off` 與 `style` 沒有對應的顯示分支，直接吐原始 JSON。

MIE 測試 178 個。`mie.js?v=38`、`mie.css?v=18`。

**㉛ 「存這段」按鈕，以及「載不進來的那個 log 其實是空的」**

使用者回報捲軸還是載不進 log，並問「現在都是要在 cmd line 按 q 才會 log 存檔?」

**兩個都不是 bug，但兩個都是我的錯。**

1. **log 一直都是即時寫入的**（`buffering=1`，獨立寫入緒）。按 q 只是補上最後那行 `summary`。這件事沒有任何地方講過。
2. **他載入的檔案裡真的沒有音。** 他 10:50:44 重啟引擎，然後直接去開檔案對話框、挑了最新的那個檔 —— 那正是他剛開起來、還沒彈過任何東西的 session：

```
10:47:56  149s  人類音 176  生成音 211   已收尾   ← 他彈的那段
10:50:44  372s  人類音   0  生成音   0   還在跑   ← 他載入的那個
```

兩個 session 的 MIDI port 都正常開啟，不是接線問題。畫布畫不出東西是**正確的**，但它看起來跟壞掉一模一樣。

**修法一：清單要講內容，不要講檔案大小。** 一個跑了六分鐘沒人彈的 session 是一個塞滿 snapshot 的大檔，從大小看起來就像你要的那一趟。`/api/logs` 現在回報每個檔的人類音／生成音數量，選項寫「09/09 10:47:56 176 音 / 引擎 211」，沒有彈奏的直接寫「（沒有彈奏）」並且淡化。掃 39 個檔案要 67 ms。

**修法二：「存這段」按鈕（Ctrl+S）。** 使用者要的是「不用離開 MIE、不用等測試結束回 console 按 q」。`EventLog.rotate()` 把目前這段收尾成一個獨立檔案（補上 summary），然後立刻開始錄下一段。

- **段落邊界是精確的**：換檔動作發生在**寫入緒上**，由佇列裡的一個標記觸發 —— 所以按下按鈕之前排進佇列的每一行都還在舊檔，之後的都在新檔。不是「大約在按下去的地方」。
- **同一秒內按兩次不會併進同一個檔**（`default_path` 會加序號），否則「存這段」會把兩趟還給你一個檔。
- 按下去之後**面板會自動把剛存好的那一段載進捲軸**，按鈕上也寫出存成什麼檔、到此幾個音 —— 事件流在演奏中一秒就捲過去了，講在按鈕上才看得到。
- 實機驗證：連按三次得到三個獨立檔案，人類音 87 / 30 / 9，捲軸自動載入第一個並畫出 185 個音；捲軸關著時按也能用。

**順手收掉一個「兩份真相」**：ws_server 原本自己組 log 路徑，現在直接 `from .eventlog import LOG_DIR` —— 清單不可能指到寫入端不在的地方。

`summary` 裡的 stats 是**引擎啟動以來**的累計而不是這一段的，所以那一行標了 `"cumulative": True`；檔案自己的事件才是這一段的真相（`mie_replay` 與捲軸都只讀事件）。

兩個測試：段落邊界必須精確（收尾檔拿到 60–64、新檔拿到 70–72，兩個都有自己的 session 與 summary）、同一秒兩次儲存不得落在同一個檔。MIE 測試 180 個。

`mie.js?v=40`、`mie-roll.js?v=7`。

**㉜ 回放送 MIDI；密度門檻用量的改**

**密度門檻 2.0 → 2.5，用掃的不是用猜的。** 2.0 在 2026-09-09 11:xx 那四趟**全部都會觸發**，而使用者對那四趟沒有意見 —— 45 秒窗口很容易碰到 2.0，整趟平均卻只有 0.92–1.82。把所有錄音掃過一遍：

```
                     整趟   窗口中位   p90    最大
22:42（說「整體變密了」） 2.33    2.70   3.34   3.94
11:04（沒抱怨）          1.82    1.30   2.02   2.44
11:11（沒抱怨）          1.40    1.24   1.48   1.54
11:12（沒抱怨）          0.92    1.64   2.59   2.85
11:15（沒抱怨）          1.28    1.10   1.24   1.33
```

**2.5 把它們完全分開**：在抱怨過的那趟觸發 21 次，在沒抱怨的四趟一次都不觸發。直接把真實錄音餵進 `Advisor`（不是重放，重放會生出它自己的音）驗證：四趟全部安靜，22:42 在 t=44 觸發。順帶把 `SPARSE_AT` 0.35 → 0.25 —— 0.35 在一趟使用者滿意的錄音裡講過一次話。

**回放送 MIDI（`Engine.play_take` / `stop_take` + 捲軸上的 🔊 開關）。** 使用者說捲軸「可以看見捲動 → no sound」，那是對的：捲軸本來只是畫圖。現在它可以把那一段真的送回樂器。

刻意做得很少，因為這是**往硬體送音**：

- **音符一次送給引擎，由 Scheduler 排程。** 逐音在播放頭經過時才送，等於把瀏覽器的 animation frame 和一條 WebSocket 放進時序裡 —— 這個專案的 jitter 是用個位數毫秒在算的。
- **回放的音永遠不會餵回邊的圖。** 否則就是「回放讓引擎去回應回放」，一個有好名字的 feedback loop。（`_on_sent` 對 `replay` lane 不觸發 `_fire_edges`。）
- **永遠不送到使用者正在彈的 channel。** 那條規則存在的理由是引擎不能跟演奏者的手打架，回放不是打破它的理由。
- **PANIC 與 BYPASS 一樣切斷它**（同一個 scheduler、同一個 `_emit`），而且**引擎在 PANIC 狀態時拒絕開始回放**。
- **預設關著**，按鈕寫「🔇 靜音回放」，打開才變「🔊 送出 MIDI」並轉成橘色。
- 只送**接下來 45 秒**、而且**只送你看得到的 lane**（圖例上關掉的不會發聲）。拖動時間軸、改速度、循環繞回去，都會重新對準引擎。

**引擎必須說出它不能發聲。** 第一版在 PANIC 之後捲軸還在捲、喇叭圖示還亮著，而每一次按下去都被靜默拒絕 —— 畫面在動而沒有聲音出來，正是這個面板存在的目的要防止的事。現在 PANIC / BYPASS 會讓按鈕變成「🔇 PANIC 中」並且 disabled，RESUME 之後自己恢復。

五個測試：送得出去而且自己收乾淨、回放不得觸發邊的圖、不得送到演奏者的 channel、stop 與 PANIC 都要收乾淨（且 PANIC 後拒絕開始）、以及 0.5× 真的把時間拉長一倍。實機驗證：關著時按播放送出 0 個訊息；打開後 7–8 個音上線；PANIC → 按鈕變 disabled、旗標歸零、紅底橫幅出現；RESUME → 全部復原、橫幅消失（這也順便證明了 `[hidden]` 那個修正兩個方向都work）。

MIE 測試 185 個。`mie.js?v=42`、`mie-roll.js?v=9`、`mie.css?v=20`。

**㉝ 回放連自己彈的一起送；以及把「互補密度」的三個提案拿去量**

**回放你彈的部分（跟著圖例上的「你」）。** 使用者的理由是對的：聽不到自己彈了什麼，就沒辦法判斷引擎的回應合不合理或突兀。

引擎有一條硬規則「絕不送到演奏者正在彈的 channel」——它存在的理由是**引擎不能跟琴上的手打架**。回顧的時候琴上沒有手，所以這是唯一該放行的情況，而且放得很窄：只有 `replay` lane 可以，只有面板明講的時候才會，`play_take` 是唯一設得出這個 lane 的地方。另外加了一個測試釘住「一般生成音仍然到不了 human channel」。

**面板行為**：圖例上的「你」還亮著 → 你彈的那部分送回你的琴；把它關掉 → 跟其他 lane 一樣安靜。實測（同一份錄音，兩次請求）：

```
你 亮著 (human=true)   接受 172 音、跳過 0，實際送出 ch9×8  ch11×7  ch10×6
你 關掉 (human=false)  接受 104 音，              ch11×7  ch10×6   ← ch9 消失
```

---

**外部提案的三個密度門檻方案，全部拿去掃**（同樣的錄音，同樣的 4 秒評估間隔）：

```
設定                       22:42(抱怨)  11:04  11:11  11:12  11:15
現在 2.5 絕對值 連2次(8s)         21       0      0      0      0
提案 1.8 連3次(12s)              34       1      0      5      0
提案 1.8 連4次(16s)              32       0      0      4      0
動態 1.5x／前3分鐘 連3次           0       0      0      0      0
動態 1.8x／前3分鐘 連3次           0       0      0      0      0
```

- **1.8 + 12 秒比現況差**：抓得比較多，但在使用者滿意的錄音裡誤報 1 次與 5 次。
- **動態自適應基準（方案 B）在所有錄音上都是 0 次，包括使用者親口抱怨過的那一趟。** 原因很直接：22:42 那趟是**整趟都密**（整趟 2.33、窗口中位 2.70），滾動基準會跟著一起漲，比值永遠到不了 1.5。**一個自我參照的基準，對「從頭到尾都太滿」是盲的**——而那正是唯一一次真的被抱怨的情況。

所以維持 2.5 絕對值 + 連兩次。

**「互補密度」（人彈得越密、引擎越退讓）**：先量現況——**14 趟裡有 12 趟，引擎是跟著演奏者一起變密**（20 秒窗口的相關係數 0.39–0.97）。這在結構上是必然的：shadow / echo / follow / phrase 全都由人的音觸發，音進得多、音就出得多。

而**這個機制其實已經存在**：`restraint(human_energy, curve) = (1-energy)^curve`，加上 `restraint` 這個全域混合比例。但量過之後它做不到提案要的事：

```
restraint 0.8 / curve 1.0   gen/human 2.34   引擎 3.85 note/s   相關 0.83
restraint 1.0 / curve 2.0             2.10        3.44         0.86
restraint 1.0 / curve 3.0             2.01        3.27         0.80
restraint 1.0 / curve 5.0             1.55        2.48         0.72
```

**`restraint_curve` 控制的是引擎「講多少」，不是「要不要跟著你」**——它把總量壓下來（2.34 → 1.55），相關係數卻幾乎不動（0.83 → 0.72）。原因也很直接：restraint 調的是每個觸發事件的機率，而觸發事件的數量本身就跟著人的音數走；機率減半、觸發加倍，輸出持平，翻不了號。

真正的互補行為需要一個新機制（例如讓 `density` 隨人的瞬時音密度反向縮放），那是**引擎性格的改變、而且是自動的**，所以先量再說，沒有動手。

MIE 測試 186 個。`mie.js?v=43`、`mie-roll.js?v=10`。

**㉞ Bregman 音域折算：量了三種，三種都更差；互補密度：量到底了**

**音域重疊折算（核准實作，但實測後沒有上）。** 提案是 `門檻 = 2.5 × (1 - 0.4 × 重疊率)`。實測結果是它讓密度訊號**變差**：

```
門檻算法                        22:42(抱怨)  11:04  11:11  11:12  11:15
固定 2.5（現在）                       21       0      0      0      0
2.5×(1-0.4×總重疊)                    37       2      0      6      0
2.5×(1-0.4×獨立 lane 重疊)             28       0      0      3      0
```

原因在前提：

- **總重疊率在每一趟都差不多**（48–86%，多數 64–81%），拿它當折算因子等於把門檻整體下修一個常數，誤報就回來了。
- **獨立音域 lane 的重疊率跟密度抱怨是反相關的**：抱怨密度那趟只有 **39%**，而使用者滿意的幾趟反而是 80–93%。折算會在最需要它的那趟把門檻**調高**，在不需要的幾趟**調低** —— 正好相反。

再試一次更貼近 Bregman 原意的算法（生成音響起當下，±2／±5 半音內有沒有還在響的人類音）：抱怨那趟 ±2 是 48%，使用者滿意的四趟是 35–49% —— **完全重疊，分不開。**

遮蔽理論本身沒有問題，但**在這位演奏者的資料上，音域重疊這個數字不帶有「他何時嫌太滿」的資訊**。所以密度門檻維持固定 2.5，不做折算。音域擁擠本來就有自己的訊號（訊號 C），它在 22:16 那趟（弦樂高音抱怨）正確觸發過。

**互補密度：量到底。** 新增 `density_complement` 全域（**預設 0 = 完全不改變行為**），公式 `有效 density = density × (1 - α × 人類瞬時密度)`。A/B（22:42 與 11:04 兩趟）：

```
變體                      gen/human    r    引擎 note/s   你最忙時 你/引擎   忙時失聲窗口
baseline（現在）               2.34   0.83      3.85       2.66 / 6.19          0
complement 1.0               2.12   0.84      3.50       2.66 / 5.62          0
restraint 1.0 curve 3        2.01   0.80      3.27       2.66 / 4.88          0
restraint 1.0 curve 8        1.21   0.47      1.94       2.66 / 2.25          0
restraint 1.0 curve 15       0.75   0.01      1.14       2.66 / 0.70          2
curve 8 + complement 1.0     1.15   0.50      1.83       2.66 / 2.20          0
```

**兩個問題都有答案了：**

1. **相關係數能不能轉負？** 能降到 0（`restraint_curve` 15，r = 0.01），但**不是靠 `density_complement`** —— 它把 r 從 0.83 推到 0.84，等於沒動。原因跟 restraint 同構、只是低一層：`density` 控制的是「一條 lane 每次發言講幾個音」，而出音量最大的 lane（shadow、follow、echo 第一趟）**每個觸發就是一個音**，跟這個旋鈕無關。**輸出量由觸發次數主導，而觸發次數就是你的音數。**
2. **代價多少？** 要完全脫鉤（r≈0）要付出 **70% 的出音量**，而且**開始出現「你彈得最兇的時候引擎完全失聲」**（忙時失聲窗口 0 → 2）。折衷點大概在 `curve 8`：r 減半（0.83 → 0.47）、出音量減半、**失聲 0 次**。

`density_complement` 留著（預設關、有測試釘住預設不改變行為、文件寫清楚它做不到什麼），因為它確實會把多音 lane 變薄；但真正能動 r 的槓桿是**本來就存在的 `restraint_curve`**，不需要新程式碼。要不要調成 curve 8，是聽感決定，不是數字決定。

MIE 測試 187 個。

**㉟ 演奏者音域避讓（`below_player`）：pad 待在你手底下，你往上跑它就往下讓**

需求：「MIE 的最高音應該低於我的主要演奏最高音，並且保留安全間隔，而不是單純把 High 設成一個很大的固定值」。這是對的——固定天花板做不到這件事，因為**同一趟裡演奏者自己的最高音就會移動兩個八度**，`high: 88` 在他彈中音域時就等於「在我上面」。

`below_player: N` = 永遠比你**當下**彈的最高音再低 N 個半音。

**三個做對才有用的細節**：

1. **動態上限必須同時進到 late binding。** `edge_range()` 原本只讀靜態 low/high，而送出前的 voice leading 會在那個範圍內重挑音高——上限只擋在提案端的話，聲部導向會把音又推回去。現在 `edge_range(edge, st)` 是**唯一**的定義，提案端與 late binding 讀同一個。
2. **「你彈到哪裡」要讀手指，不讀踏板。** 沿用 22:16 那趟的教訓：`sounding` 含踏板，它的最大值在踏板踩著時只會往上長。`player_top()` 先看 `held`，完全放開時才退回最近 6 秒的最高音——這樣抬手時 pad 不會突然彈上去。
3. **地板是硬的，只有天花板會動。** 第一版讓地板跟著往下滑，實測兩趟的中位數掉到 **E3 / C3**——那不是 pad 讓開，那是變成一條低音 drone 跑到左手底下去。pad 有自己的音域；演奏者低到沒有空間時，它待在地板上而不是往下鑽。

**還有一條**：pad 不會等音符自然結束才讓開。`below_player` 開著時，每個 tick 會把**已經高過天花板**的音放掉（留 2 個半音的邊界避免抖動）——「當我的演奏音域往上移動時，MIE 自動向下重新配置」。聲部預算滿了要收音時，也改成**優先收最高的那個**（原本收最舊的），對應「優先刪除或降低最高聲部」。

**A/B（兩趟真實錄音重放）**：

```
                                 音數  中位   最高   高於你的比例   中位間隔
110415  修改前 (high 88, hold 6)   27   D5    D6      88.9%      -10.0 半音
110415  新設定                     26   C4    F4       3.8%       +6.0 半音
221627  修改前 (high 88, hold 6)   58   F5    D6      50.0%       -1.5 半音
221627  新設定                     52   C4    A4       3.8%      +15.0 半音
```

**scene 的改動**：`below_player: 7`、`high: 88 → 79`（沒有輸入可讀時的硬底線，G5）、`hold_beats: 6 → 4`（和弦跟得上演奏）、`release_beats: 2.0 → 1.5`（尾音不堆積）。`low` 維持 55，`voices` 與 `vel` 沒動。核心和弦辨識與輸入處理完全沒碰。

面板上新增「讓開(半音)」滑桿，所以這是可以邊彈邊調的，不是只能改檔案。`below_player` 與 `above_held` 講的是相反的事（待在我下面／坐在我上面），所以**指定的那個贏**，另一個直接不看——同時做兩件相反的事只會兩件都做不準。

兩個測試：天花板要跟著演奏者上下、地板不准動、再低也不能塌成空範圍、沒設這個參數時行為完全不變；以及演奏者爬上去之後，pad 要主動把高過天花板的音放掉。MIE 測試 189 個。

**㊱ `below_player` 推廣到任何一條 lane；面板照著捲軸的顏色上色**

**1. 跟隨（橘色）也會跑到你上面。** 實測 11:15 那趟：**81% 的 follow 音高過演奏者當下的最高音**。原因是 `edge_range()` 只在邊有設 `low`/`high` 時才回傳範圍，而 follow / shadow / echo 都沒有自己的音域——所以那條天花板對它們根本沒有作用。

Follow 本來就是「在你的音上面一個五度」，所以你彈到接近自己的上限時它必然超出。改法不是丟掉那些音（那會讓這條 lane 消音），而是給它一個天花板加上兩個八度的空間，交給 late binding：**音級保留、往下移一個八度**——上面的五度就變成下面的四度。

```
每條 lane 有多少比例的音高過你當下的最高音（11:15 那趟）
                        follow  sustain  shadow  echo  phrase
只有 sustain 讓開           81%     17%      4%    7%    16%
follow 也 below_player 5     3%     17%      4%    8%    14%
```

scene 設 `follow_iridium_5th.below_player = 5`，81% → 3%。shadow / echo / phrase 沒設，因為它們重複的本來就是你自己彈的音高（4–16% 多半是你在它們響著的時候往下移了），而且那是它們的工作。「讓開(半音)」滑桿現在是**每條邊都有**的共通參數，不是 sustain 專屬。

**2. 面板照著捲軸的顏色上色。** 捲軸分得清楚，但互動邊清單全部同一個顏色，看到一條橘色的音無法對到是哪一條邊。現在：

- 每一條邊的左側色條 = 捲軸上那條 lane 的顏色（**同一個定義**：`window.MieRoll.hueFor()`，不是抄一份會走鐘的色表）
- 演算法藥丸改成顯示 **lane 名稱**（影子／回音／跟隨／樂句／延續／織體），跟捲軸圖例用同一組字；滑上去才顯示原本的 algo
- 事件流的 GEN 行也帶上 lane 顏色與名稱

**一個做完才發現的問題**：`.mie-edge.hot`（剛觸發的黃框）會把左側色條一起蓋掉——而那正是你盯著看想知道「這是哪個顏色」的時刻。`hot` / `is-mute` / `is-waiting` / `off` 現在只吃另外三邊，左側色條永遠是 lane 的顏色。實測八條邊在 hot / off 狀態下色條都正確。

MIE 測試 190 個。`mie.js?v=45`、`mie.css?v=22`。

**㊲ 13:36 / 13:39 兩趟：讓開全部生效；但也暴露了兩個我自己造出來的問題**

**兩個修正都上機確認：**

```
                     跟隨   延續   影子   回音   樂句     ← 高過你當下最高音的比例
11:04（修改前）        80%    93%    5%    1%    18%
11:11（修改前）        86%    88%    3%    7%    19%
11:15（修改前）        92%   100%    0%    6%     4%
13:36（新設定）         0%     0%    6%    5%    16%
13:39（新設定）         0%     0%    4%    4%     3%
```

`sustain` 中位數 C4 / D4（範圍 A3–E4、G#3–A4），`follow` 中位數 F3 / G3。兩趟都是 0 迴圈 / 0 靜音 / 0 錯誤，jitter p95 0.92 與 1.6。

**問題一：丟棄從 0 變成 3 和 5，全部是 `follow` 撞到 constraint。** 你彈很低的時候，`below_player` 的天花板把窗口壓到樂器下限附近，那幾個半音裡可能一個合法音級都沒有，`snap()` 回 None，音就被丟掉了。

**音域是偏好，不是否決權。** 稍微超出被要求的音域，比這條 lane 靜靜地把音吃掉要小得多——後者正是這個專案一路在修的那類 bug。`late_bind` 現在在被 `note_range` 收窄過、而且找不到落點時，退回樂器自己的音域再試一次。重放這兩趟：`constraint` 丟棄 3 → **0**、5 → **0**，而讓開的效果沒有變（follow 仍是 3%）。

**問題二：落差提示開始在這兩趟講「sustain 有 100% 的音落在你雙手之間」——而那正是它被要求做的事。** 訊號 C 量的是「落在你 10–90 百分位音域裡的比例」，一個待在你右手底下、左手上面的 pad，按這個定義就是「在你雙手之間」。**我自己的修正把這條提示變成了假警報**，而且它給的建議（「讓它讓開」）是叫一條已經在讓開的 lane 再讓一次。

現在：**設了 `below_player` 的邊不再被這條提示點名**——它已經被明確告知要待在哪裡，再去質疑它就是在推翻使用者自己下的指令。順帶把這條提示的修正動作改成設 `below_player: 7`（原本是把八度往上推，那只是把問題搬家）。八趟錄音重跑，兩趟新的都安靜了。

三個測試：音域是偏好不是否決權（一個容不下任何和弦音的窗口不得把音丟掉）、讓開的 lane 不得被點名、以及原本那些斷言都還在。MIE 測試 191 個。

**㊳ 13:56 / 13:59 兩趟：丟棄回到 0；follow 讓開 5 → 3**

**上一輪的兩個修正都上機確認**：兩趟都是 **0 丟棄**（「音域是偏好不是否決權」生效）、0 靜音、0 迴圈、0 錯誤，jitter p95 1.47 / 1.53。落差提示兩趟都安靜（讓開中的 lane 不再被點名）。

使用者的判斷是對的：`follow` 中位數掉到 F3 / **C3**，「很乾淨但聽不出它在跟」。所以 5 → 3，其餘一律不動。

**量法要對**：第一次我用「近 6 秒的最高音」當比較基準，但規則跟的是**當下手指按著的音**（`player_top()` 先讀 `held`）。用對的基準重量：

```
135623                   follow音  中位   離手指 p25   中位    p75   高過手指
  讓開關掉                    26   D#5      -19     -12     +1      73%
  讓開 3                     26   D#3       +5     +12    +17       8%
  讓開 5                     26   C#3       +5     +14    +17       8%
135934
  讓開關掉                    33    A3       -7     +16    +24      39%
  讓開 3                     33    E3      +16     +18    +24       0%
  讓開 5                     33    D3      +17     +22    +26       0%
```

**5 → 3 讓中位距離縮短 2–4 個半音，而高過手指的比例沒有回升**（8% / 0%）。135623 那趟有四分之一的 follow 音落在你手指下方 5 個半音之內——那個距離聽得出是在跟。丟棄在所有設定下都是 0。每人類音 1.24 / 1.21 完全不受這個參數影響（它改的是音高，不是音量）。

**一個要講清楚的限制**：135934 那趟即使讓開 3，中位距離仍是 +18 半音。這不是 gap 太大，是**取音方式**：`snap(..., "nearest")` 從「你的音 + 五度」往下找最近的合法音級，落在天花板下方 0–11 個半音的任意處。真要讓 follow 貼著天花板走，該改的是「優先取窗口頂端」而不是「最近的」——那會影響所有 lane，所以先量不先做。

**順帶發現**：scene 現在套著**敬拜風格**（tension 0.2 / density 0.35 / time 1.5 / prob_scale 0.5 / restraint 1.0），所以 `sustain_strings` 的 `hold_beats` 被風格改回 **12**、`release_beats` 回到 **3.0**——先前依要求設的 4 / 1.5 被覆蓋了。風格照設計運作，但那項要求等於被取消了，記在這裡以免以後查不到。

MIE 測試 191 個。

**㊴ 「影子／跟隨有時飆高音」：天花板算對了，但在錯的時刻算，而且根本套不上去**

先釐清一件事：14:07 那趟的 log header 是 `below_player: None`——**那趟根本還沒套上新設定**，follow 飛到 C7 是舊行為。14:11 那趟有新設定，follow 就正常了。

真正的兩個 bug 是這樣被找出來的：

**一、音域範圍是在「排程時」算好存進 NotePair 的。** `note_range` 在排程當下算好、存在音符上；而 echo 有延遲、phrase 隔好幾秒才回來——**等它響的時候，你的手早就移走了**。那正是 `below_player` 存在的理由，而它讀的是過期的天花板。所以這個設定看起來幾乎沒作用：把 echo 上限設成 +12，「超過一個八度」的比例只從 12% 動到 11%。現在只有設了 `below_player` 的邊會在**送出當下**重算，其餘照舊、不付任何代價。

**二、`snap()` 只搜尋 ±12 半音。** 一個離窗口三個八度的音**永遠搆不到**——`snap` 回 None，而我上一輪加的「找不到就放寬」機制就把它原封不動送出去了。**上限看起來套上了，實際上什麼都沒做。** 現在被 `below_player` 收窄過時，會先用**八度**把音摺進窗口再 snap：音級是 echo 必須保留的東西，音域正是它可以讓的。

這個摺疊**只在被 `below_player` 收窄時**才做，絕不套用在樂器自己的音域上——把 octave 拉到 −3 讓整條 lane 掉到合成器下限，那是要看得見的丟棄，不是該被悄悄摺回可聽範圍的東西（19:27 那趟的事故）。

**`below_player` 現在接受負值**，代表「最多高過你這麼多」：`-12` = 不超過你手上一個八度。對 echo / phrase 這種「重複你彈過的音」的 lane 這才是對的形狀——高半個八度還是回音，高三個八度就是隔壁房間。

**A/B（兩趟真實錄音，數字是相對於當下按著的最高音）**：

```
                 echo              shadow            phrase            follow           sustain      丟棄
14:07 全部關掉  中位-2 最高+39   中位+0 最高+11   中位+5 最高+28   中位-5 最高+32   中位-6 最高+19    0
14:07 現在設定  中位-1 最高+12   中位+0 最高 +1   中位+5 最高+12   中位-17最高+19   中位-14最高 +6    0
14:11 全部關掉  中位-2 最高+27   中位+0 最高 +9   中位-3 最高+24   中位+14最高+43   中位+9 最高+33    0
14:11 現在設定  中位-2 最高+12   中位+0 最高 +1   中位-3 最高+10   中位-17最高+24   中位-6 最高 +9    0
```

**中位數幾乎沒動、最高值被壓下來**——這正是要的：echo 還是在你的音高上回音，只是不再跑到三個八度以上；shadow 中位仍是 **+0**（它就是在複製你的聲部），但**再也不會因為避碰撞而往上跳一個八度**（最高 +11 → +1）。總生成音數幾乎不變（665→641、283→294），沒有東西被丟掉，丟棄全程 0。

scene：`echo -12`、`phrase -12`、`echo2 -12`、`shadow -1`（不高過你，但不強迫它降八度——設成 +1 時 shadow 中位掉到 −12，整整低一個八度，那不是影子）。

**方案 A 也做了**：敬拜風格的 `sustain_strings` 從 `hold_beats 12 / release 3.0` 改成 **6 / 2.0**——保留鋪底的空氣感，但和弦切換時不再拖那麼久。

四個測試：音域要在音「響的時候」讀而不是排程時、負值代表向上的間隔、被收窄時要用八度摺進來、以及樂器音域的丟棄行為不准變。MIE 測試 193 個。
...
**㊶ 14:35 / 14:39 兩趟；「手動調過的就是你的」——選風格不再洗掉你調好的東西**

兩趟都健康：0 丟棄、0 靜音、0 迴圈、0 錯誤，jitter p95 1.51 / 1.63。每人類音 1.58 / 1.22。`follow` 中位 E3 / B3、`sustain` 中位 B3 / C#4——讓開的規則穩定在位。

**而 log 直接指出了那個 bug**（14:39 那趟）：

```
master_gain=0.35   @243s   ← 使用者手動調的音量
prob_scale=0.5     @259s   ← 259 秒套用「流行」
max_hop=2          @259s
avoid_semitone=... @259s   ← 連這些流行風格根本沒設的東西都被寫回去
tension=0.2        @259s
```

流行風格只設 5 個 global，但整組全域都被覆寫。原因是我先前為了「風格不要疊加」加的 `_restore_style_base()`：**切換風格前先回到套用前的基準**——而那個基準是套風格當下的完整快照，於是使用者在那之後手調的一切都被丟掉。

**改成效果器的行為**：使用者**親手**動過的設定就是他的，風格不能寫它，回到基準也不能。

- `set_global` / `set_edge` 在**不是 bulk 操作**時記下路徑（`_bulk` 本來就是「這是批次不是使用者動作」的旗標）。
- `apply_style` 與 `_restore_style_base` 跳過這些路徑。
- **preset 不受影響**——A/LIVE/B 是「叫回我存好的整個狀態」，那就該整組回來，有測試釘住。
- **`退回檔案` 與切換 scene 會全部交還**，那是唯一意思是「忘掉我做過的事」的動作。

**面板要看得見**，否則規則是隱形的——一條風格默默跳過的滑桿，跟一條被默默覆寫的滑桿一樣讓人困惑：

- 手動調過的滑桿名稱變成強調色並加一個 ● ，滑鼠移上去說明為什麼
- 頂列出現「我調過 N」，tooltip 列出是哪幾項，按下去全部交還
- **點滑桿名稱**可以單獨交還某一項
- 事件流會寫「風格 → pop（8 條邊，保留你調過的 2 項）」

實機驗證：套敬拜 → 手調音量 0.35、張力 0.7 →（標記出現、徽章「我調過 2」）→ 改流行 → **厚度與時間跟著風格走，音量與張力原封不動** → 再改爵士 → 仍然不動 → 點「張力」交還 → 再套敬拜 → 張力這才被風格移動、音量仍是你的 → 按徽章 → 全部交還、徽章消失。

四個測試。MIE 測試 199 個。`mie.js?v=48`、`mie.css?v=23`。

**㊷ 15:00 那趟；follow 的 interval 回到 +7，改由天花板去壓**

15:00 那趟很乾淨：441 人類音 / 484 生成、每音 1.10、**0 丟棄 / 0 靜音 / 0 迴圈 / 0 錯誤**、jitter p95 1.6。**持久化上機生效**——引擎自己開在 `test01`、`hold_beats 6`，沒有再手動選；`last_session.json` 記著 `{"scene":"test01","style":"pop"}`。

**讓開規則的實測**（相對於當下按著的最高音）：follow 中位 −17、只有 1% 高過；shadow 中位 +0、最高 **+1**；echo 與 phrase 最高剛好卡在 **+12**（就是設定的上限）。

`echo` 33%、`sustain` 32% 這兩個「高過你」的比例看起來嚇人，但量下去幅度很小、時間很短：

```
sustain   8 個  高出中位 +5 半音  響 0.96 s
echo     59 個            +5 半音  響 0.20 s
phrase    8 個            +2 半音  響 0.33 s
```

**原因也量出來了：整趟有 62% 的時間演奏者手上什麼都沒按著。** 他是一句一句彈的，所以「手指下的最高音」多數時候不存在，規則只能退回最近 6 秒。這不是飄在上面，是樂句之間的空檔裡短暫高幾個半音。等實際聽感回報再決定要不要縮短那個回溯窗口。

**follow 的 interval 從 −8 改回 +7**（演奏者先前自己改成小六度往下；現在要它回到五度往上，由 `below_player` 去壓）。preset A 裡也有一份，一併改，否則按 A 就會退回去。

```
                              音數  中位     範圍     離手指p25  中位  p75  高過你  貼在7半音內
15:00  interval -8              88  G#3   C2–F5      +13   +19  +24    2%     15%
15:00  interval +7 + 讓開 3      88   A3   C2–A5      +10   +17  +24    1%     19%
15:00  interval +7 + 讓開關掉     88  A#4   D2–B6      -15    -2  +24   51%     52%
14:39  interval -8              90  C#4   C2–B5       +7   +16  +19    0%     33%
14:39  interval +7 + 讓開 3      90   C4   C2–C6       +6   +16  +22    0%     33%
14:39  interval +7 + 讓開關掉     90   E5   C2–C7      -24    +2  +35   50%     54%
```

**音域幾乎和 −8 時一樣，甚至更貼近一點**（p25 +13 → +10）——因為決定它待在哪裡的是天花板，不是那個音程。而把天花板關掉就看得出原始的 +7 是什麼樣子：**51% 的音在演奏者上面**。天花板確實在做事。
